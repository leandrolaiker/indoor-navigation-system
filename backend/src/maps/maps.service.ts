...
import { MapDocument, MapEntity, NavigationStep } from './schemas/map.schema';
import { CreateMapDto } from './dto/create-map.dto';
import { SaveGraphDto } from './dto/save-graph.dto';
import { ImageGenerationService } from '../image-generation/image-generation.service';
import { hashPasos } from '../image-generation/navigation-instructions.util';
...
export class MapsService {
  ...
  async saveGraphAndGenerateImages(id: string, dto: SaveGraphDto): Promise<MapDocument> {
    ...
    floor.pois = dto.pois.map((p) => {
      const existing = floor.pois.find((e) => e.id === p.id);
      return {
        id: p.id,
        name: p.name,
        x: p.x,
        ...
        generatedImagePath: existing?.generatedImagePath ?? null,
        instructions: existing?.instructions ?? null,
        textoGenerado: existing?.textoGenerado ?? null,
        instructionsMode: existing?.instructionsMode ?? 'auto',
        manualInstructions: existing?.manualInstructions ?? null,
        manualInstructionsHash: existing?.manualInstructionsHash ?? null,
      };
    });
    ...
    for (const poi of floor.pois) {
      ...
      if (generatedInstructions.has(poi.id)) {
        const generated = generatedInstructions.get(poi.id);
        const pasos = generated?.pasos;
        poi.instructions = pasos
          ...
          : null;
        poi.textoGenerado = generated?.texto ?? null;
      }
    }
  ...
  /**
   * Guarda el texto de instrucciones editado a mano para un POI.
   * Vaciar el texto (null o string vacío) hace que el POI vuelva a modo
   * 'auto' automáticamente (opción B acordada), descartando el texto
   * manual guardado.
   */
  async updatePoiInstructions(
    ...
    if (!trimmed) {
      poi.instructionsMode = 'auto';
      poi.manualInstructions = null;
      poi.manualInstructionsHash = null;
    } else {
      poi.instructionsMode = 'manual';
      poi.manualInstructions = trimmed;
      poi.manualInstructionsHash = poi.instructions ? hashPasos(poi.instructions) : null;
    }
   ...
   * renombran después de generar las instrucciones). Si el POI está en
   * modo manual, `texto` es el editado a mano, y `desactualizado` indica
   * si la ruta cambió después de haberlo guardado.
   */
  private buildInstructionsResponse(
    ...
    const esManual = poi.instructionsMode === 'manual' && Boolean(poi.manualInstructions);
    const desactualizado =
      esManual && poi.manualInstructionsHash !== hashPasos(poi.instructions);
    ...
    return {
      ...
      textoGenerado: poi.textoGenerado,
      instructionsMode: poi.instructionsMode,
      manualInstructions: poi.manualInstructions,
      texto: esManual ? poi.manualInstructions! : poi.textoGenerado,
      desactualizado,
    };
  }
}
