...
/**
 * Genera un color HSL determinístico a partir del id de la zona: mismo id
 * siempre da el mismo color. Es la misma función (misma fórmula) que
 * `colorForZone` en `frontend/src/components/CanvasEditor.tsx`, para que
 * el color de cada zona coincida entre el editor y la imagen PNG
 * generada. Se duplica en vez de compartirse porque frontend y backend
 * son paquetes npm independientes en este MVP (sin un paquete `shared`
 * común); si se agrega uno más adelante, es candidato a moverse ahí.
 */
function colorForZone(id: string): { fill: string; stroke: string; text: string } {
  let hash = 0;
  ...
  return {
    ...
    stroke: `hsla(${hue}, 70%, 40%, 0.85)`,
    text: `hsla(${hue}, 70%, 30%, 0.95)`,
  };
}
  ...
  private drawRouteImage(
    ...
    if (zones.length > 0) {
      ...
      for (const zone of zones) {
        const color = colorForZone(zone.id);
    ...
    for (let i = 0; i < routePois.length - 1; i += 1) {
      ...
      // esta ruta recorremos la arista al revés (b -> a en el guardado),
      // hay que invertirlos, o el trazo conecta los waypoints en el
      // orden equivocado (el último queda pegado a la entrada y el
      // primero al POI de destino, en vez de al revés).
      const orderedWaypoints = edge
        ? edge.fromPoiId === a.id
          ? edge.waypoints
          : [...edge.waypoints].reverse()
        : [];
      const segmentPoints = [{ x: a.x, y: a.y }, ...orderedWaypoints, { x: b.x, y: b.y }];
