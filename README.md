...
**6. Nuevo: cada zona con su propio color, y referencias de "pasar cerca de".**
- Cada zona ahora se dibuja con un color propio (calculado a partir de su id, no elegido a mano), consistente entre el editor y la imagen PNG generada.
- Las instrucciones de navegación ya no solo detectan cuando la ruta pasa **por dentro** de una zona: si pasa **cerca** del borde de una zona (a menos de 40px del plano) sin entrar en ella, se genera un paso `pasar_cerca_de` con esa zona como referencia. Si hay varias zonas cerca a la vez, se usa la más cercana (no la más chica, a diferencia del caso "por dentro"). Ver "Cómo se integran con la generación de rutas" en la sección **"Zonas"**.

**7. Fix: los giros ahora distinguen "cerca" de "dentro" en el JSON.**
En la primera versión de "pasar cerca de", un giro (`girar_derecha`, `dar_vuelta`, etc.) que ocurría cerca de una zona sin entrar en ella le pegaba igual la `referencia` al giro, exactamente igual que si hubiera ocurrido dentro de la zona — el JSON no traía forma de distinguir un caso del otro. Ahora, cuando un giro ocurre cerca (no dentro) de una zona, se emite un paso `pasar_cerca_de` propio justo antes del giro, y el giro queda sin `referencia`. Así, cualquier `referencia` que aparezca directamente sobre un giro siempre significa "dentro" de esa zona/POI, y "cerca" siempre es su propio paso identificable por `tipo`. Ver la tabla de "Cómo distinguir 'por dentro' de 'cerca'" en la sección **"Instrucciones de navegación (JSON)"**.
...
- Test funcional aislado de `findNearbyZoneAt()`: se verificó que un waypoint fuera de toda zona pero a menos de 40px de una genera el paso `pasar_cerca_de`; que si hay varias zonas cerca gana la más CERCANA (no la más chica); y que un waypoint a más de 40px de cualquier zona no genera ninguna referencia.
- Test de la función `colorForZone`: se confirmó que es determinística (mismo id → mismo color en llamadas repetidas) y que ids distintos producen matices distintos.
- Test funcional de `buildNavigationInstructions()` con un giro cerca de una zona ("Evento") seguido de otro giro dentro de una zona distinta ("Salón"): se verificó que el primero genera un paso `pasar_cerca_de` separado (con el giro sin `referencia`) y el segundo adjunta `referencia` directamente al giro — confirmando que ambos casos ya son distinguibles por `tipo` en el JSON.
...
7. Verificar también que la imagen PNG generada (miniatura en el panel derecho) muestra el recuadro de la zona dibujado sobre el plano, con un color propio (no siempre el mismo violeta), y que ese color coincide con el que se ve en el editor.
...
9. **Pasar cerca (sin entrar).** Editar el punto de control de la conexión para que quede *fuera* del recuadro de la zona pero a poca distancia del borde (unos 40px del plano o menos), guardar de nuevo, y confirmar que la instrucción para ese tramo ahora dice `"tipo": "pasar_cerca_de"` con el nombre de la zona como referencia, en vez de no decir nada.
10. Opcional: dibujar una segunda zona cerca de ese mismo punto (a distinta distancia que la primera) y confirmar que la referencia usada es la de la zona **más cercana**, no la más chica.
...
5. **Agregar zonas.** Elegir "🟪 Agregar zona" y arrastrar sobre el plano para dibujar un recuadro (ej. sobre un salón completo o un sector de pasillo), luego escribir su nombre. A diferencia de un POI (un punto), una zona ocupa un área: cualquier tramo de ruta que pase por dentro se referenciará con el nombre de la zona en las instrucciones de navegación (aunque no haya un POI puntual ahí), y un tramo que pase cerca sin entrar genera una referencia "pasar cerca de". Doble clic sobre una zona la renombra. Cada zona se dibuja con un color propio para diferenciarlas.
...
```json
{
  ...
  "pasos": [
    ...
    { "tipo": "avanzar", "distancia": 60.0 },
    { "tipo": "pasar_cerca_de", "referencia": "Depósito" },
...
- `pasar_por` — sigue derecho, pero pasa junto a un POI o **por dentro** de una zona con nombre (se usa como referencia sin cortar el tramo en dos).
- `pasar_cerca_de` — el trazado corre a menos de 40px del plano del borde de una zona, sin entrar en ella (ej. un pasillo paralelo a una sala). Ver la sección **"Zonas"** más abajo.
- `llegada` — último paso, siempre con `referencia` = nombre del POI destino.

**Cómo distinguir "por dentro" de "cerca" en el JSON — siempre por el `tipo` del paso, nunca por un campo extra:**
- Si el tramo **sigue derecho**: `pasar_por` = dentro del POI/zona; `pasar_cerca_de` = cerca sin entrar. Son dos pasos de `tipo` distinto, uno u otro, nunca el mismo paso con ambos significados.
- Si el tramo **gira** (`girar_*` / `dar_vuelta`) justo en un POI o dentro de una zona, la referencia se adjunta directamente al giro (`{ "tipo": "girar_derecha", "referencia": "Salón" }`) — es el equivalente implícito a un "pasar por" combinado con el giro.
- Si el tramo **gira** cerca de una zona pero sin entrar en ella, se emite un paso `pasar_cerca_de` **separado, justo antes** del giro, y el giro en sí queda **sin** `referencia`: `{ "tipo": "pasar_cerca_de", "referencia": "Evento" }` seguido de `{ "tipo": "dar_vuelta" }`. Así, un giro con `referencia` propia siempre significa "dentro", y una referencia "cerca" siempre aparece como su propio paso `pasar_cerca_de`, se esté por girar ahí o no.

`referencia`, cuando aparece, es el nombre del POI o zona asociado a ese punto exacto de la ruta (nunca el de un waypoint sin nombre y sin zona cerca).
...
Cada zona se dibuja con un color propio, calculado de forma determinística a partir de su `id` (mismo id → siempre el mismo color; zonas distintas → colores distintos, sin necesidad de guardar un campo de color en la base de datos ni elegirlo a mano). La función que genera el color (`colorForZone`, un hash simple del id proyectado a un matiz HSL) está duplicada a propósito en `frontend/src/components/CanvasEditor.tsx` y `backend/src/image-generation/image-generation.service.ts`, con la misma fórmula, para que el color de una zona en el editor coincida exactamente con el de la imagen PNG generada.

### Cómo se integran con la generación de rutas

Cada vez que se guarda el grafo, además de las imágenes e instrucciones ya existentes, el cálculo de instrucciones (`navigation-instructions.util.ts`) consulta las zonas como *fallback* de referencia, en dos niveles:

1. Se calcula la ruta más corta (POIs + waypoints) como siempre, sin que las zonas influyan en el pathfinding.
2. **Pasar por dentro.** Al construir los puntos de la ruta, cada waypoint que **no** es un POI con nombre se consulta contra las zonas con `findZoneAt(zones, x, y)`: si cae dentro de alguna, se le asigna el nombre de esa zona y pasa a comportarse igual que un POI con nombre en el resto del pipeline (no se descarta al simplificar el trazado, y genera `pasar_por` o el `referencia` de un giro). **Si dos zonas se solapan**, gana la que tiene **menor área** (ej. un "Baño" puntual dentro de un "Pasillo Este" más grande se reporta como "Baño", la referencia más específica).
3. **Pasar cerca (sin entrar).** Si el waypoint no cae dentro de ninguna zona, se consulta `findNearbyZoneAt(zones, x, y)`: si el borde de alguna zona está a menos de **40px del plano** (`NEARBY_ZONE_THRESHOLD_PX`), se guarda ese nombre como referencia "débil". Este es, por ejemplo, el caso de un pasillo que corre paralelo a una sala sin entrar en ella. **Si hay varias zonas cerca a la vez, gana la más CERCANA** (no la más chica: acá lo relevante es la proximidad, no el área). A diferencia de un waypoint común, estos puntos tampoco se descartan al simplificar el trazado, porque el caso típico es justamente un tramo recto (sin giro) paralelo a la zona — si se descartaran, se perdería la referencia en el caso más común.
4. Al narrar el tramo: si el punto tiene referencia por "estar dentro" (`name`), se emite `pasar_por` en un tramo recto, o esa referencia se adjunta directamente al paso de giro (`girar_*`/`dar_vuelta`) si ahí hay un cambio de dirección. Si sólo tiene referencia por "estar cerca" (`nearbyName`), se emite **siempre** un paso `pasar_cerca_de` como paso propio — tanto en un tramo recto como justo antes de un giro (en ese caso, el giro en sí queda sin `referencia`). Esto es a propósito: así, un giro que trae `referencia` en el JSON siempre significa "dentro", y "cerca" siempre es identificable por su propio `tipo: "pasar_cerca_de"`, sin ambigüedad ni campos extra que inspeccionar. "Estar dentro" siempre tiene prioridad sobre "estar cerca" en un mismo punto.
5. Cada punto de la ruta se evalúa de forma independiente, así que si el trazado pasa dentro de una zona en un tramo y cerca de otra en un tramo distinto, ambas pueden aparecer como pasos separados de la misma ruta.

Ejemplo de instrucciones con los tres casos en la misma ruta — cerca en un tramo recto, cerca justo antes de un giro, y dentro en un giro:

```json
{
  "pasos": [
    { "tipo": "avanzar", "distancia": 120 },
    { "tipo": "pasar_cerca_de", "referencia": "Sala de Reuniones" },
    { "tipo": "avanzar", "distancia": 40 },
    { "tipo": "pasar_cerca_de", "referencia": "Evento" },
    { "tipo": "dar_vuelta" },
    { "tipo": "avanzar", "distancia": 80 },
    { "tipo": "girar_derecha", "referencia": "Salón" },
    { "tipo": "avanzar", "distancia": 60 },
    { "tipo": "llegada", "referencia": "Oficina 12" }
  ]
}
```

Notar la diferencia: `"Evento"` (cerca, sin entrar) aparece como su propio paso `pasar_cerca_de` justo antes del `dar_vuelta`, que queda sin `referencia`; `"Salón"` (el giro ocurre dentro de la zona) aparece directamente como `referencia` del `girar_derecha`.

Esto da contexto adicional útil para un servicio que redacte el texto final (ej. una integración con Ollama), incluso en tramos donde no hay ningún POI puntual.

### En la imagen generada

Las zonas también se dibujan en la imagen PNG de cada POI (antes que la ruta resaltada, para no taparla): un recuadro semitransparente con su nombre, con el mismo color por zona que en el editor (ver "Color por zona" arriba).
...
El DTO de guardado (`SaveGraphDto.zones?: ZoneDto[]`) es opcional, así que mapas guardados antes de esta entrega (sin zonas) siguen funcionando sin cambios: `floor.zones` queda como `[]`. El color no se persiste como campo: se deriva del `id` en tiempo de render/generación (ver arriba), así que tampoco requiere migración.
...
- El umbral de "pasar cerca de" (`NEARBY_ZONE_THRESHOLD_PX = 40`) es un valor fijo en píxeles del plano subido, igual para todos los mapas. No se ajusta automáticamente según la escala real del edificio (un plano de alta resolución "ve" el umbral más chico en términos relativos que uno de baja resolución). Si hace falta, es un solo valor para exponer como parámetro configurable por mapa.
