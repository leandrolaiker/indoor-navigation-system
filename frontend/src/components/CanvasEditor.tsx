 ...
 * siempre da el mismo color (estable entre renders y entre sesiones),
 * pero distinto id da (casi siempre) un color distinto. Se replica la
 * misma función en el backend (`image-generation.service.ts`) para que
 * el color de cada zona en el editor coincida con el de la imagen PNG
 * generada. No se persiste como campo propio en el modelo de datos: se
 * deriva del id para no tener que migrar mapas ya guardados.
 */
function colorForZone(id: string): { fill: string; stroke: string } {
  let hash = 0;
